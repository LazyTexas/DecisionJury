...
作用：
  1. 读取 data/history_records.json 中已有的历史记录。
  2. 用确定性的模板组合补足到 1000 条（购物 500 + 时间 500）；
     购物侧覆盖电子数码、学习办公、生活家电、服饰、运动户外、食品、图书、美妆个护、
     游戏、收纳、出行，以及日用品相关品类（清洁用品、纸品耗材、厨房用品、个护清洁、
     家纺布艺、防护日用、文具耗材），尽量覆盖多种类日用品。
...
CATEGORY_PROS = {
    ...
    "清洁用品": ["日常消耗快,实用", "明显提升清洁效率", "性价比高", "家里常备不浪费"],
    "纸品耗材": ["消耗快,囤货划算", "生活必需,不闲置", "单价低,试错成本小", "提升生活便利"],
    "厨房用品": ["做饭效率提升", "日常使用频率高", "清洗方便", "提升下厨体验"],
    "个护清洁": ["日常消耗,复购稳定", "使用体验好", "适合长期使用", "改善个人状态"],
    "家纺布艺": ["提升居住舒适度", "使用周期长", "换季刚需", "改善居家氛围"],
    "防护日用": ["日常出行实用", "价格低,试错成本小", "消耗品不闲置", "解决具体小痛点"],
    "文具耗材": ["学习办公刚需", "消耗快不闲置", "单价低", "提升书写/整理效率"],
}
...
CATEGORY_CONS = {
    ...
    "清洁用品": ["容易囤太多", "占储物空间", "品牌差异大,易踩雷", "用量估算不准"],
    "纸品耗材": ["囤太多占地方", "受潮或存放问题", "规格容易买错", "促销时容易冲动加购"],
    "厨房用品": ["厨房空间有限", "与现有厨具重复", "清洗保养麻烦", "使用频率不确定"],
    "个护清洁": ["肤质/发质不适配", "容易囤积过期", "同类产品太多", "效果因人而异"],
    "家纺布艺": ["尺寸容易买错", "清洗晾晒麻烦", "占收纳空间", "价格跨度大易超预算"],
    "防护日用": ["容易随手多买", "质量参差", "家里同类太多", "使用频率不高"],
    "文具耗材": ["容易囤太多", "与已有文具重复", "质量参差", "实际用量被高估"],
}
...
# 主流程
# ---------------------------------------------------------------
# 旧版本生成器遗留的文案：其中的“用品”会让「学习用品」这类查询命中大量无关日用品，
# 这里做一次幂等归一化，保证重复运行也不会再出现该词。
LEGACY_CONTENT_REPLACEMENTS = {
    "成为日常高频用品。": "使用频率很高，成了日常离不开的东西。",
}


def normalize_legacy_content(records):
    for record in records:
        content = record.get("content")
        if not isinstance(content, str):
            continue
        updated = content
        for old, new in LEGACY_CONTENT_REPLACEMENTS.items():
...
def main():
    ...
    records = normalize_legacy_content(unique_records)
