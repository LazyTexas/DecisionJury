...
def test_reminder_id_format():
    ...
    result = create_reminder(user_id="u001", case_id="c001", title="test", cooling_days=1)
...
def test_days_less_than_one():
    """cooling_days < 1 should be clamped to 1"""
    result = create_reminder(user_id="u001", case_id="c001", title="test", cooling_days=0)
    due = datetime.strptime(result["due_at"], "%Y-%m-%dT%H:%M:%S+08:00")
    now = datetime.now()
...
def test_days_negative():
    """negative cooling_days should be clamped to 1"""
    result = create_reminder(user_id="u001", case_id="c001", title="test", cooling_days=-5)
    due = datetime.strptime(result["due_at"], "%Y-%m-%dT%H:%M:%S+08:00")
    now = datetime.now()
...
def test_missing_user_id():
    ...
    result = create_reminder(user_id="", case_id="c001", title="test", cooling_days=3)
    assert result["status"] == "error"


def test_missing_case_id():
    """missing case_id should return error"""
    result = create_reminder(user_id="u001", case_id="", title="test", cooling_days=3)
    assert result["status"] == "error"


def test_create_and_validate_keys():
    """Ensure all expected keys are present"""
    result = create_reminder(user_id="u001", case_id="c001", title="冷静测试", cooling_days=2, reason="测试用")
