...
首次构建需要拉取基础镜像并安装依赖，视网络情况约 3~10 分钟。
后续只改代码不依赖时，重建会快很多（依赖层有缓存）。

## 4. 常用命令速查

> 前提：Docker Desktop 已启动（任务栏鲸鱼图标变绿）。以下命令都在项目根目录执行。
...
docker compose ps                 # 查看状态，三个都 healthy 才算就绪
...
|---|---|---|---|
| backend | python:3.12-slim | Debian 13 | `bash`、`sh` |
| rag | python:3.12-slim | Debian 13 | `bash`、`sh` |
...
| 变量 | 默认值 | 说明 |
|---|---|---|
| `DEEPSEEK_API_KEY` | 空 | **必填**。不填则正反方/解析全走 mock，演示会"不智能" |
| `DEEPSEEK_TIMEOUT_SECONDS` | 30 | LLM 请求超时 |
...
> ⚠️ 这三处跨服务地址如果沿用默认值 `127.0.0.1`，容器之间会互相连不上。
> 源码里已经全部走 `os.getenv`，compose 覆盖即可，无需改代码。
...
- 把本地已有的 `data/decisionjury.db` 灌进容器（可选）：

```bash
docker compose up -d backend
docker cp data/decisionjury.db $(docker compose ps -q backend):/data/decisionjury.db
...
完整链路（含 RAG 与 MCP 工具）的自动化验证脚本见 `rag/e2e_verify.py`，
需在容器内执行：

```bash
docker compose exec backend python -m rag.e2e_verify   # 注意路径，按脚本实际位置调整
```
...
|---|---|---|
| 前端 | vite dev server（8088） | nginx 托管静态构建产物（80 → 宿主机 8088） |
...
看 `docker compose logs backend`。常见原因：`.env` 没建（`DEEPSEEK_API_KEY` 为空不影响健康检查，但会影响"智能"）、RAG 未 healthy 导致 backend 未启动。

**Q2：前端能打开但接口全 404 / 登录失败？**
说明 nginx 少配了一条 location。本项目后端有两条前缀：业务是 `/api`，认证是 `/auth`（**没有** `/api` 前缀），`frontend/nginx.conf` 里两条都要有。
...
这是 Windows 的端口保留问题（不是端口被占用）。Hyper-V/WSL 会预留一批 TCP 段，落在段内就无法 bind。
先查保留段：
...
- 辩论链路调用 `cooling_reminder` 后未落库到 `reminders` 表，
  因此前端"观察清单"页在真实链路里为空。
- `GET /api/cases/{case_id}/messages` 尚未实现，前端用 localStorage 缓存对话。
- time 决策主流程未接入，前端"时间决策"入口点进去走不通。

以上三项已同步给后端 / Agent 编排，修复后重新 `docker compose up -d --build` 即可生效。
