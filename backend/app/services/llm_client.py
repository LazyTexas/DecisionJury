...
# 真实 API 失败时返回的降级标记：调用方据此把"模型没回答"如实标出来，
# 而不是把 mock 模板文案当成正方/反方的真实论点（T08：静默降级=假装）。
DEGRADED_REASON_KEY = "_degraded_reason"
...
class DeepSeekLLMClient:
    ...
    def complete_json(self, task: str, payload: dict[str, Any]) -> dict[str, Any]:
        ...
        for attempt in range(2):     # 失败重试一次：网络抖动不该直接降级成模板文案
            try:
            ...
            except Exception as exc:   # noqa: BLE001 —— 任何失败都要能被观测并以 mock 兜底
                last_error = exc
                logger.warning("LLM 调用失败 task=%s attempt=%d/%d: %s: %s",
                               task, attempt + 1, 2, type(exc).__name__, exc)
        # 真实 API 失败不能让 Agent 主流程崩溃，但**必须留下降级标记**：
        # 调用方要么如实标注"模型未回答"，要么走本地兜底，绝不能把 mock 模板当成模型观点。
        fallback = self.fallback_client.complete_json(task, payload)
        fallback[DEGRADED_REASON_KEY] = f"{type(last_error).__name__}: {last_error}"[:200]
        return fallback
...
def debate_context_fields(fields: dict[str, Any] | None) -> dict[str, Any]:
    """辩论/判决上下文里的字段视图。

    `description` 是用户**最先**说的那句话，可能已被后续对话更正（真实事故：
    "想买3袋" 后来被纠正为"不是3袋，是2袋"，字段已是 2，但模型同时看到原话里的 3，
    于是在判决书里写出"数量口径存在矛盾"。矛盾是喂进去的，不是用户造成的）。
    因此这里把原话显式标注为历史信息，数值一律以结构化字段为准。
    """
    view = {k: v for k, v in (fields or {}).items() if k != "description"}
    raw = (fields or {}).get("description")
    if isinstance(raw, str) and raw.strip():
        view["原始描述（用户最先说的话，可能已被后续对话更正；数值以以上字段为准）"] = raw.strip()
    return view
...
def _build_system_prompt(task: str) -> str:
    ...
    if task == "input_parser":
        return (
            ...
            "判定口径是【功能上是否还能用】，与用户想不想要新的无关："
            "用户说“还能用/没坏/还可以/洗洗能用/还能凑合/旧的还能使” → true（现有物品仍能满足用途）；"
            "用户说“坏了/不能用了/开胶/鼓包/失效/充不进电/已经不行了”或功能明显不足 → false；"
            "只是提到有旧物、没说能不能用 → null。拿不准就给 null，不要猜，也不要用“用户想换新”当作覆盖不了的理由。\n"
            ...
            "- ask_field：**必填**，声明 question 问的是哪个字段，只能从服务端给的 askable_fields 里选一个；"
            "askable_fields 为空、或你本轮不问问题时，question 与 ask_field 都必须给空字符串。\n"
            "  question 必须与 ask_field 指向同一件事：**不允许**就字段表以外的话题发问"
            "（例如“轻便还是力度”“几天内到手”“尽快收到还是慢慢挑”——这些答案系统没有字段可存，属于无效追问）。\n"
            ...
            "服务端只允许你就 askable_fields 里的字段提问（askable_fields 为空表示信息已够、该收尾了）："
            "此时应当承接一句 + 告诉用户可以直接出判决书，而不是自己发明新维度继续追问。\n"
            "返回字段应包含 case_type、is_supported、is_high_risk、reject_reason、extracted_fields、"
            "correction_fields、confidence、dialogue；next_question / next_question_key 是**已废弃的兼容字段**，"
            "留空即可——问句只能写在 dialogue.question，追问目标只能写在 dialogue.ask_field，服务端只认这两个。"
        )
    ...
    return (
        ...
        "结构化字段是用户确认后的权威状态；“原始描述”只是用户最先说的话，可能已被后续对话更正，"
        "数值与结论一律以结构化字段为准，不得用原始描述里的旧数字与字段值构造矛盾。\n"
        + (
        ...
        "required_output": (
            {
                ...
                "alternative_covers_need": "boolean or null（用户已有同类物品时：功能上是否还能用——"
                                           "“还能用/没坏/洗洗能用”为 true，“坏了/开胶/鼓包/失效”为 false；"
                                           "只是提到旧物、没说能否使用给 null）",
...
def _coerce_confidence(raw: Any) -> float | None:
    """宽松解析模型给的 confidence；认不出数字返回 None。

    真实事故（用户实测日志）：模型把 confidence 写成 `"0.85（较高）"` / `"85%"` 这类形式，
    严格 `float()` 直接判整轮 LLM 结果作废（`LLM result confidence is not numeric`），
    触发重试甚至降级成兜底文案。这里只做"能认出数字就认"的容错：
    纯数字 / 数字字符串 / 带百分号 / 数字夹在说明文字里都能取出来；布尔与 NaN 仍视为无效。
    """
    ...
    if not text:
        return None
    match = _CONFIDENCE_NUMBER.search(text)
    if not match:
        return None
    try:
...
def _validate_llm_result(value: Any) -> dict[str, Any]:
    ...
    confidence_number = _coerce_confidence(confidence)
    if confidence_number is None:
        raise ValueError("LLM result confidence is not numeric")
...
def _sanitize_dialogue(raw: Any) -> dict[str, Any] | None:
    ...
    if intent not in DIALOGUE_INTENTS:
        intent = "provide_info"
    ask_field = raw.get("ask_field")
    ask_field = ask_field.strip()[:40] if isinstance(ask_field, str) else ""
    return {
        "ack": _text("ack", 60),
        "insight": _text("insight", 80),
        "question": _text("question", 80),
        "ask_field": ask_field,          # 模型声明的追问目标；服务端只接受落在 askable 集合里的值
        "chips": chips,
...
def _validate_parser_result(value: Any) -> dict[str, Any]:
    ...
    if confidence is None:
        raise ValueError("parser confidence is not numeric")
