...
在代码基线 `dev@23c492c` 上执行全量测试，实际结果如下：

```text
命令：uv run --frozen pytest -p no:cacheprovider tests -q --tb=line --show-capture=no
结果：279 passed, 18 failed, 7 warnings in 39.02s
```
...
| 失败范围 | 数量 | 主要原因 |
|---|---|---|
| `test_cases_router.py` 案件查询与报告 | 5 | JWT 接入后用户身份校验逻辑变化，测试仍按旧的公开访问或旧返回结构断言 |
| `test_chat_router.py` 消息列表缺少 user_id | 1 | 新路由在缺少身份信息时的返回与旧测试预期不一致 |
| `test_history_router.py` 历史查询缺少 user_id | 1 | 同上，缺少用户身份时的错误码/状态码未同步测试 |
| `test_watchlist_router.py` 观察清单缺少 user_id | 1 | 同上，缺少用户身份时的返回与旧测试预期不一致 |
| `test_trace_route.py` 轨迹查询 | 5 | 轨迹接口接入用户校验后，测试数据缺少用户关联，返回结构与旧断言不一致 |
| `test_migrate.py` 迁移测试 | 5 | 测试 fixture 使用独立引擎，而被测迁移函数使用 `backend.migrate.engine`，导致表不存在 |

需要说明的是，这些失败并不表示 Agent、RAG、工具等核心模块不可用，而是说明 JWT 改造后部分路由测试和迁移测试的隔离方式尚未同步更新。团队在文档基线 `53d70bc` 上的历史记录为 `292 passed, 5 failed`，本次 `23c492c` 全量结果为 `279 passed, 18 failed`，失败数量增加主要来自 JWT 接入后的路由断言变化，属于答辩前需要优先修复的测试同步问题。
...
3. 全量测试仍有 18 个失败，集中在 JWT 接入后的路由断言和迁移测试隔离问题，答辩前需要由 B 模块和测试负责人优先修复。
4. 本轮未执行真实 DeepSeek API、浏览器完整闭环和 Docker 构建验收，报告中不将 mock 结果写成真实模型结果。
5. 建议在答辩前完成：路由测试与 JWT 行为对齐、迁移测试引擎隔离修复、两个购物案例的浏览器全流程截图、真实 DeepSeek 调用记录。
...
需要如实说明的是，本轮范围最终收敛为购物决策，时间决策主流程延期；全量测试仍有 18 个失败，集中在 JWT 接入后的路由断言和迁移测试隔离问题；真实 DeepSeek API、浏览器完整闭环和 Docker 构建还需要在答辩前补充验收。这些内容不写成“已经全部完成”，而是作为后续改进项保留。
...
| 全量测试仍有 18 个失败 | JWT 接入后部分路由测试断言未同步；迁移测试 fixture 与被测函数使用不同引擎 | 统一路由鉴权测试写法，修复迁移测试引擎隔离，补充 JWT 强制/兼容模式用例 |
